 ...
 *   <li><code>GET /admin/batch/status</code> - 배치 시스템 상태 확인</li>
 * </ul>
 */
...
public class BatchController {
	...
	/**
	 * CSV(장소) 파일을 데이터베이스로 적재하는 배치 작업을 시작합니다.
	 *
	 * <p>data/csv/ 디렉토리에 위치한 장소 관련 CSV 파일을 읽어 DB에 저장합니다.
	 * 이 엔드포인트는 비동기 실행되며, 호출 즉시 배치가 백그라운드에서 진행됩니다.</p>
	 *
	 * <h4>특징</h4>
	 * <ul>
	 *   <li><b>HTTP</b>: POST /admin/batch/csv-to-db</li>
	 *   <li><b>응답</b>: 202 Accepted (수락 후 비동기 실행), 본문에 execId 포함</li>
	 ...
	 * <h4>주의사항</h4>
	 * <ul>
	 *   <li>대용량 데이터로 인해 처리 시간이 길 수 있습니다.</li>
	 *   <li>동일 시각에 여러 번 호출 시 중복 실행이 차단될 수 있으며(409 Conflict), 재시도 필요합니다.</li>
	 *   <li>다중 인스턴스 운영 시에는 Redis 분산 락 등 추가 장치가 필요할 수 있습니다.</li>
	 * </ul>
	 *
	...
	public ResponseEntity<CustomApiResponse<?>> runCsvToDbBatch() {
		if (!placeBatchRunning.compareAndSet(false, true)) {
			return ResponseEntity.status(409)
				.body(CustomApiResponse.error(ErrorCode.CONFLICT, "이미 실행 중입니다."));
		}
		...
		Thread t = new Thread(() -> {
			try {
				...
				log.info("[배치] 장소 CSV → DB 적재 시작 (execId={})", execId);
				jobLauncher.run(placeCsvJob, params);
				log.info("[배치] 장소 CSV → DB 적재 완료 (execId={})", execId);

			} catch (Exception e) {
				log.error("[배치] 장소 CSV → DB 적재 중 오류 발생 (execId={})", execId, e);
			} finally {
				placeBatchRunning.set(false);
				MDC.clear();
			}
		}, "batch-place-" + execId);
		...
		return ResponseEntity.accepted()
			.body(CustomApiResponse.success("배치 작업이 시작되었습니다. execId=" + execId));
	}
